...
const router = useRouter()
  ...
  return (
    <header className="min-h-20 py-6">
      <div className="max-w-7xl h-full mx-auto px-6 md:px-4 lg:px-0 flex justify-between items-center">
        ...
        <div className="flex justify-between flex-1 items-center">
          <nav className="md:flex ml-6  items-center hidden">
            {mainLinks.map((link) => (
              <span
                key={link.name}
                className="text-sm px-3 mx-2 transition-all duration-700 hover:text-gray-900 hover:bg-slate-300 rounded-lg py-2 font-medium text-gray-700 uppercase"
              >
        ...
        <Link href="/contact" passHref>
          <button className="ml-6 px-4 py-2 shadow-md  bg-green-600 hover:bg-green-700 text-white text-base uppercase rounded-full font-medium">
            Get Started
          </button>
        </Link>
      </div>
